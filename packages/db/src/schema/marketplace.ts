 ...
 *
 * The token is stored as a SHA-256 HASH, never in the clear. A leaked database should not
 * hand somebody every outstanding invitation, and the plaintext exists only in the message
 * that was sent — exactly like a password reset link.
 */
export const rfqInvite = pgTable(
  ...
  (t) => [
    unique("uq_rfqinvite_broadcast_supplier").on(t.tenantId, t.broadcastId, t.supplierId),
...
export const notificationOutbox = pgTable(
  ...
  (t) => [
    ...
    index("ix_outbox_tenant_related").on(t.tenantId, t.relatedType, t.relatedId),
  ],
);
...
export const networkQuoteSubmission = pgTable(
  "network_quote_submission",
  {
    ...
    toolingCost: numeric("tooling_cost", { precision: 18, scale: 2 }).notNull().default("0"),
