 ...
 * that already cost the Northstar e-invoice step; do not "tidy" these back to fromToday().
 *
    ...
    async () => {
      const res = await buyer(
        ...
        "/api/v1/purchase/rfqs",
        {
          ...
          needDate: fromToday(30),
          quoteDeadline: fromToday(7),
          deliveryPlant: "Pune",
  ...
  const responses = [
    {
      ...
      body: { unitPrice: 1920, toolingCost: 0, freightCost: 14000, nonCreditableTax: 0,
              promisedDate: fromToday(26), leadTimeDays: 26, moq: 60 },
    },
    {
      key: "v2",
      vendor: v2,
      ...
      body: { unitPrice: 1840, toolingCost: 18000, freightCost: 9000, nonCreditableTax: 0,
              promisedDate: fromToday(24), leadTimeDays: 24, moq: 120 },
    },
    {
      key: "v3",
      vendor: v3,
      ...
      body: { unitPrice: 1710, toolingCost: 0, freightCost: 7500, nonCreditableTax: 0,
              promisedDate: fromToday(42), leadTimeDays: 42, moq: 240 },
    },
  ];
    ...
    await step("a second person awards it, with the reason written down → purchase order", async () => {
      const res = await approver(
        ...
        `/api/v1/purchase/rfqs/${rfq.id}/award`,
        {
          quoteId: winner.id,
          awardReason:
            "Meets the need date with certification; the cheaper quote cannot deliver before the build and the dearest costs ₹19,200 more landed.",
        },
        "demo-rfq-award",
      );
