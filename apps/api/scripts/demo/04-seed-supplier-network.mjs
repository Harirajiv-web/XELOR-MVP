...
async function main() {
  ...
  for (const s of created.suppliers) {
    ...
    await step(`${s.name} opens the link`, async () => {
      const res = await fetch(`${process.env.API_BASE ?? "http://localhost:3300"}/api/v1/supplier/${tok}`);
    ...
    await step(`${s.name} sends a price`, async () => {
      const res = await fetch(
        `${process.env.API_BASE ?? "http://localhost:3300"}/api/v1/supplier/${tok}/quote`,
        {
          method: "POST",
      ...
      if (res.status === 409) return { note: "already answered (replay)" };
      if (!res.ok) throw new Error(body?.error?.message ?? `HTTP ${res.status}`);
      created.answered += 1;
