...
import { SOURCING_QUOTE_SINK, type SourcingQuoteSink } from "../../ports/sourcing.port.js";
import {
  NotifyService,
  renderRfqCard,
...
export class NetworkService {
  ...
  async upsertSupplier(input: SupplierInput, idempotencyKey: string): Promise<{ id: string }> {
  ...
  private async doUpsertSupplier(input: SupplierInput): Promise<{ id: string }> {
    if (!input.whatsappE164 && !input.email) {
      throw Errors.validation([
        { field: "whatsappE164", message: "a supplier needs a WhatsApp number or an email address, or they cannot be asked" },
      ]);
    }
    ...
    return withTenant(async (tx) => {
      ...
      if (existing) {
        await tx.update(networkSupplier).set(values).where(eq(networkSupplier.id, existing.id));
  ...
  private async doBroadcast(
    ...
    const { tenantId, actorId } = currentTenant();
    const base = process.env.SOURCE_PORTAL_BASE_URL ?? "http://localhost:3301";
    const ttlHours = Number(process.env.SOURCE_INVITE_TTL_HOURS ?? 336);

    const queued: string[] = [];
    const out = await withTenant(async (tx) => {
      const [rfq] = await tx.select().from(sourcingRfq).where(eq(sourcingRfq.id, rfqId)).limit(1);
      if (!rfq) throw Errors.notFound(`RFQ '${rfqId}'`);
      if (rfq.status === "awarded" || rfq.status === "cancelled") {
        throw new AppError("RFQ_NOT_OPEN", 409, `${rfq.rfqNo} is ${rfq.status} and cannot be published.`);
      }

      const all = await tx.select().from(networkSupplier).where(eq(networkSupplier.status, "active"));
      const chosen = input.supplierIds?.length
        ? all.filter((s) => input.supplierIds!.includes(s.id))
        : input.category
          ? all.filter((s) => (s.categories as string[]).includes(input.category!))
          : all;
      if (chosen.length === 0) {
        throw new AppError(
      ...
      for (const supplier of chosen) {
        ...
        if (supplier.whatsappE164) {
          targets.push({ ...common, channel: "whatsapp", recipient: supplier.whatsappE164 });
        }
        if (supplier.email) {
          targets.push({
            ...common,
    ...
    await withTenant(async (tx) => {
      for (const id of queued) await this.notify.flush(tx, id);
    });
    return out;
  }
  ...
  async openInvite(token: string): Promise<Record<string, unknown>> {
    return withTenant(async (tx) => {
      ...
      if (!invite.openedAt) {
        await tx
          .update(rfqInvite)
          .set({ openedAt: new Date(), state: invite.state === "sent" ? "opened" : invite.state })
      ...
      return {
        ...
        state: submission ? "quoted" : invite.state === "sent" ? "opened" : invite.state,
  ...
  async submitQuote(token: string, input: SubmitQuoteInput): Promise<{ ok: true; landedCost: string }> {
    const { tenantId, actorId } = currentTenant();
    const queued: string[] = [];
    ...
    const out = await withTenant(async (tx) => {
      ...
      const landed =
        round2(input.unitPrice) + round2(input.toolingCost ?? 0) + round2(input.freightCost ?? 0);
      ...
      const [supplier] = await tx
        ...
        .where(eq(networkSupplier.id, invite.supplierId))
        .limit(1);
      const [rfq] = await tx.select().from(sourcingRfq).where(eq(sourcingRfq.id, invite.rfqId)).limit(1);
      ...
      if (rfq) {
        const link = `${process.env.SOURCE_PORTAL_BASE_URL ?? "http://localhost:3301"}/sourcing/detail/${rfq.id}`;
        const body = renderQuoteReceivedCard({
          ...
          landedCost: m2(landed),
          promisedDate: input.promisedDate ?? null,
          meetsNeedDate: input.promisedDate ? input.promisedDate <= rfq.needDate : null,
          link,
        });
        queued.push(
          await this.notify.queueInTx(tx, {
            channel: "whatsapp",
            recipient: process.env.SOURCE_BUYER_WHATSAPP ?? "+919000000001",
            recipientName: "Sourcing desk",
            template: "quote_received",
            body,
            variables: { rfqNo: rfq.rfqNo, supplier: supplier?.name, landedCost: m2(landed) },
            linkUrl: link,
      ...
      return { ok: true as const, landedCost: m2(landed) };
    });
  ...
  private async loadInvite(tx: Tx, token: string): Promise<typeof rfqInvite.$inferSelect> {
    ...
    if (invite.expiresAt.getTime() < Date.now()) {
      throw new AppError("INVITE_EXPIRED", 410, "This invitation has expired. Ask the buyer for a new link.");
    }
    return invite;
  }
   ...
   */
  async acceptSubmission(submissionId: string, idempotencyKey: string): Promise<{ quoteId: string }> {
    const result = await runIdempotent(
      idempotencyKey,
      fingerprint({ submissionId, op: "accept-submission" }),
      async () => ({ status: 201, body: await this.doAccept(submissionId, idempotencyKey) }),
    );
    return result.body;
  }

  private async doAccept(submissionId: string, idempotencyKey: string): Promise<{ quoteId: string }> {
    const { actorId } = currentTenant();
    const prepared = await withTenant(async (tx) => {
      const [s] = await tx
        .select()
        .from(networkQuoteSubmission)
      ...
      if (s.sourcingQuoteId) {
        throw new AppError("ALREADY_ACCEPTED", 409, "This answer is already a quote on the request.");
      }
      const [supplier] = await tx
        .select()
        .from(networkSupplier)
      ...
      if (!supplier?.vendorId) {
        throw new AppError(
          ...
          409,
          `${supplier?.name ?? "This supplier"} is on the network but is not yet an approved vendor, so no purchase order could be raised against them. Approve them as a vendor first.`,
        );
      }
      return { submission: s, vendorId: supplier.vendorId };
    });

    // Through RfqService, so the quote is recorded exactly as a typed one would be —
    // invitation check, revisioning, landed cost, audit.
    const view = await this.rfqs.recordQuote(
      prepared.submission.rfqId,
      {
        vendorId: prepared.vendorId,
        unitPrice: Number(prepared.submission.unitPrice),
        toolingCost: Number(prepared.submission.toolingCost),
        freightCost: Number(prepared.submission.freightCost),
        ...(prepared.submission.promisedDate ? { promisedDate: prepared.submission.promisedDate } : {}),
        ...(prepared.submission.leadTimeDays !== null
          ? { leadTimeDays: prepared.submission.leadTimeDays }
          : {}),
      },
      `${idempotencyKey}:quote`,
    );

    const live = view.quotes.find((q) => q.vendorId === prepared.vendorId && q.status === "submitted");
    if (!live) throw new AppError("QUOTE_NOT_RECORDED", 500, "The quote was not recorded on the request.");
  ...
  async listSubmissions(rfqId: string): Promise<Record<string, unknown>[]> {
    return withTenant(async (tx) => {
      ...
      const suppliers = ids.length
        ? await tx.select().from(networkSupplier).where(inArray(networkSupplier.id, ids))
        : [];
   ...
   */
  async rank(rfqId: string): Promise<{ needDate: string; quotes: RankedQuote[] }> {
    return withTenant(async (tx) => {
      const [rfq] = await tx.select().from(sourcingRfq).where(eq(sourcingRfq.id, rfqId)).limit(1);
      if (!rfq) throw Errors.notFound(`RFQ '${rfqId}'`);
      ...
      const promised = rows.map((r) => days(r.promisedDate)).filter((d): d is number => d !== null);
      ...
      const scored = rows.map((r) => {
        ...
        const costScore = dearest === cheapest ? 1 : (dearest - cost) / (dearest - cheapest);
        const d = days(r.promisedDate);
        const speedScore =
          d === null ? 0 : latest === soonest ? 1 : (latest - d) / (latest - soonest);
          ...
          meets === false
            ...
            : meets === null
              ...
              : cost === cheapest
                ...
                : `${money} landed — ${Math.round(((cost - cheapest) / cheapest) * 100)}% above the cheapest answer, delivering ${r.promisedDate}.`;
        ...
        return {
          ...
          explanation,
          disqualified:
            meets === false ? "Cannot deliver before the material is needed." : null,
        };
      });
  ...
  async listOutbox(limit = 100): Promise<(typeof notificationOutbox.$inferSelect)[]> {
    return withTenant((tx) =>
      tx.select().from(notificationOutbox).orderBy(desc(notificationOutbox.createdAt)).limit(limit),
    );
  }

  async listBroadcasts(): Promise<Record<string, unknown>[]> {
    return withTenant(async (tx) => {
      const rows = await tx.select().from(rfqBroadcast).orderBy(desc(rfqBroadcast.createdAt));
      const ids = rows.map((r) => r.id);
      const invites = ids.length
        ? await tx.select().from(rfqInvite).where(inArray(rfqInvite.broadcastId, ids))
        : [];
      return rows.map((b) => {
