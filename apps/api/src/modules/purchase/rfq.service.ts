...
export interface RfqView {
  ...
  status: string;
  createdBy: string;
  invitations: { vendorId: string; vendorName: string | null; responseStatus: string }[];
...
export class RfqService {
  ...
  async createRfq(input: CreateRfqInput, idempotencyKey: string): Promise<RfqView> {
  ...
  private async doCreateRfq(input: CreateRfqInput): Promise<RfqView> {
    if (input.vendorIds.length === 0) {
      throw Errors.validation([
        { field: "vendorIds", message: "invite at least one supplier — an RFQ with nobody to answer it is a note" },
      ]);
    }
    if (input.quoteDeadline > input.needDate) {
      throw Errors.validation([
        { field: "quoteDeadline", message: "the quote deadline must fall on or before the need date" },
      ]);
    }
    ...
    return withTenant(async (tx) => {
      const [it] = await tx.select({ id: item.id }).from(item).where(eq(item.id, input.itemId)).limit(1);
      if (!it) throw Errors.notFound(`item '${input.itemId}'`);

      const vendors = await tx
        .select({ id: vendor.id })
        .from(vendor)
        .where(inArray(vendor.id, input.vendorIds));
      ...
      const id = newId();
      const rfqNo = await this.numbering.next(tx, "sourcing_rfq", fyCode(now.toISOString()));
      ...
      await this.audit.appendInTx(tx, {
        ...
        entityId: id,
        data: { rfqNo, itemId: input.itemId, qty: String(input.qty), invited: input.vendorIds.length },
      });
  ...
  private async doIssue(rfqId: string): Promise<RfqView> {
    ...
    const now = new Date();
    return withTenant(async (tx) => {
      const rfq = await this.load(tx, rfqId);
      if (rfq.status !== "draft") {
        throw new AppError("RFQ_NOT_DRAFT", 409, `${rfq.rfqNo} is ${rfq.status}; only a draft can be issued.`);
      }
      await tx
        .update(sourcingRfq)
   ...
   * having moved.
   */
  async recordQuote(rfqId: string, input: RecordQuoteInput, idempotencyKey: string): Promise<RfqView> {
    const result = await runIdempotent(
      idempotencyKey,
      fingerprint({ rfqId, ...input, op: "record-quote" }),
      async () => ({ status: 201, body: await this.doRecordQuote(rfqId, input) }),
    );
    return result.body;
  }

  private async doRecordQuote(rfqId: string, input: RecordQuoteInput): Promise<RfqView> {
    const { tenantId, actorId } = currentTenant();
    const now = new Date();
    return withTenant(async (tx) => {
      const rfq = await this.load(tx, rfqId);
      ...
      const nextRevision = prior.reduce((max, q) => Math.max(max, q.revisionNo), 0) + 1;
      ...
      const landed =
        ...
        round2(input.toolingCost ?? 0) +
        round2(input.freightCost ?? 0) +
        round2(input.nonCreditableTax ?? 0);
      ...
      await this.audit.appendInTx(tx, {
        ...
        entityType: "sourcing_rfq",
        entityId: rfqId,
        data: { rfqNo: rfq.rfqNo, vendorId: input.vendorId, revisionNo: nextRevision, landedCost: m2(landed) },
      });
      return this.viewInTx(tx, rfqId);
    });
  }
  ...
  async setGate(
    ...
    const result = await runIdempotent(
      ...
      async () => ({ status: 200, body: await this.doSetGate(rfqId, quoteId, gate, note) }),
    );
    return result.body;
  }
  ...
  private async doSetGate(
    ...
    return withTenant(async (tx) => {
      ...
      const [q] = await tx
        .select()
        .from(sourcingSupplierQuote)
        .where(and(eq(sourcingSupplierQuote.id, quoteId), eq(sourcingSupplierQuote.rfqId, rfqId)))
        .limit(1);
      if (!q) throw Errors.notFound(`quote '${quoteId}' on ${rfq.rfqNo}`);
      if (gate === "conditional" && !note) {
        throw Errors.validation([
          { field: "note", message: "a conditional pass needs the deviation written down" },
        ]);
      }
      await tx
        .update(sourcingSupplierQuote)
        .set({ technicalGate: gate, gateNote: note ?? null, updatedAt: now, updatedBy: actorId })
  ...
  async award(
    ...
    const result = await runIdempotent(
      ...
      async () => ({ status: 201, body: await this.doAward(rfqId, input, idempotencyKey) }),
    );
    return result.body;
  }
  ...
  private async doAward(
    ...
    const prepared = await withTenant(async (tx) => {
      ...
      if (rfq.status === "awarded") {
        throw new AppError("RFQ_ALREADY_AWARDED", 409, `${rfq.rfqNo} has already been awarded.`);
      }
      if (rfq.status !== "evaluation" && rfq.status !== "issued") {
        throw new AppError("RFQ_NOT_AWARDABLE", 409, `${rfq.rfqNo} is ${rfq.status}.`);
      }
      ...
      const [q] = await tx
        .select()
        .from(sourcingSupplierQuote)
        .where(and(eq(sourcingSupplierQuote.id, input.quoteId), eq(sourcingSupplierQuote.rfqId, rfqId)))
        .limit(1);
      if (!q) throw Errors.notFound(`quote '${input.quoteId}' on ${rfq.rfqNo}`);
      if (q.status !== "submitted") {
        throw new AppError("QUOTE_SUPERSEDED", 409, "That quote revision has been superseded or withdrawn.");
      }
    ...
    const po = await this.purchase.createPo(
      {
        ...
        ...(input.expectedDate ?? prepared.quote.promisedDate
          ...
          : {}),
        remarks: `Awarded from ${prepared.rfq.rfqNo}. ${input.awardReason}`,
        lines: [
          {
    ...
    return withTenant(async (tx) => {
      ...
      await tx.insert(outboxEvent).values({
        ...
        payload: { id: rfqId, rfqNo: prepared.rfq.rfqNo, poId: po.id, poNo: po.poNo },
        createdAt: now,
      });
      return this.viewInTx(tx, rfqId);
    });
  ...
  async list(limit: number, cursor?: string): Promise<CursorPage<RfqSummary>> {
    return withTenant(async (tx) => {
        ...
        .where(
          keyset
            ? or(
                ...
                and(eq(sourcingRfq.createdAt, new Date(keyset.createdAt)), gt(sourcingRfq.id, keyset.id)),
              )
            : undefined,
        )
      ...
      const invites = ids.length
        ? await tx.select().from(sourcingRfqInvitation).where(inArray(sourcingRfqInvitation.rfqId, ids))
        : [];
      const awards = ids.length
        ? await tx.select().from(sourcingAward).where(inArray(sourcingAward.rfqId, ids))
        : [];
      const vendorIds = [...new Set(awards.map((a) => a.vendorId))];
      const vendors = vendorIds.length
        ? await tx.select({ id: vendor.id, name: vendor.name }).from(vendor).where(inArray(vendor.id, vendorIds))
        : [];
      ...
      return {
        items: page.map((r) => {
          ...
          return {
            ...
            quotedCount: mine.filter((i) => i.responseStatus === "quoted").length,
            awardedVendorName: award ? (vendorName.get(award.vendorId) ?? null) : null,
          };
        }),
        nextCursor:
  ...
  private async load(tx: Tx, rfqId: string): Promise<typeof sourcingRfq.$inferSelect> {
    const [row] = await tx.select().from(sourcingRfq).where(eq(sourcingRfq.id, rfqId)).limit(1);
    if (!row) throw Errors.notFound(`RFQ '${rfqId}'`);
    return row;
  }

  private async viewInTx(tx: Tx, rfqId: string): Promise<RfqView> {
    ...
    const [award] = await tx.select().from(sourcingAward).where(eq(sourcingAward.rfqId, rfqId)).limit(1);
