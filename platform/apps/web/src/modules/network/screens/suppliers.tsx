...
import { Mail, MessageCircle, Plus, TrendingUp } from "lucide-react";
...
export default function SuppliersScreen(_props: ScreenProps): React.JSX.Element {
  ...
  const [performance, setPerformance] = useState<NetworkSupplier | null>(null);
  ...
  const columns: ReadonlyArray<Column<NetworkSupplier>> = [
    {
      key: "performance", header: "Performance", width: "w-36",
      render: (supplier) => <button className="btn btn-secondary btn-sm" onClick={() => setPerformance(supplier)}><TrendingUp className="h-3.5 w-3.5" aria-hidden />View evidence</button>,
    },
    {
      key: "name",
      header: "Supplier",
  ...
  return (
    <div className="flex flex-col gap-4">
      {performance ? <SupplierPerformance supplier={performance} onClose={() => setPerformance(null)} /> : null}
      {adding ? <NewSupplier onClose={() => setAdding(false)} onSaved={() => { setAdding(false); suppliers.reload(); }} /> : null}
      <PageHeader
        title="Supplier network"
        actions={can("purchase.network.manage") ? <button className="btn btn-primary" onClick={() => setAdding(true)}><Plus className="h-4 w-4" aria-hidden />Add supplier</button> : undefined}
