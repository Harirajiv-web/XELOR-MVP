...
platform_dir = pathlib.Path(__file__).resolve().parents[3]
source_path = pathlib.Path(sys.argv[1]) if len(sys.argv) > 1 else platform_dir / "docs/reports/xelor-platform-architecture-and-verification-dossier.html"
...
out_path = pathlib.Path(sys.argv[2]) if len(sys.argv) > 2 else pathlib.Path("XELOR_Architecture_Dossier.html")
