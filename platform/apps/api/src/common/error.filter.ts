...
export class ErrorEnvelopeFilter implements ExceptionFilter {
    ...
    // Express body-parser emits plain errors before a controller runs. Preserve the
    // bounded import contract without logging the rejected body or reporting a 500.
    const parserError = exception as { type?: unknown; status?: unknown } | null;
    if (parserError && typeof parserError === "object" &&
        ((parserError.type === "entity.too.large" && parserError.status === 413) ||
         (parserError.type === "entity.parse.failed" && parserError.status === 400))) {
      const status = parserError.status as number;
      res.status(status).json({ error: { code: httpCode(status), traceId,
        message: status === 413 ? "Request body exceeds the 2 MB limit." : "Request body must contain valid JSON." } });
      return;
    }
