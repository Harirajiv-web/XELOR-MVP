...
test("each department Factory route requires its own scoped permission", () => {
  ...
  assert.deepEqual(required(prototype.factoryOperationsView), ["production.factory-connect.read"]);
  assert.deepEqual(required(prototype.simulate3sWorkroom), ["factory.scenario.execute"]);
  assert.deepEqual(required(prototype.ingestFactoryState), ["factory.telemetry.ingest"]);
});
