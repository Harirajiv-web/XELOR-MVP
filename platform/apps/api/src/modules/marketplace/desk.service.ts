import { awardBlockers } from "../../common/commercial-rules.js";
...
export interface DeskOverview {
  kpis: {
    ...
    savingsBasis: "estimated_quote_comparison";
    ...
    return withTenant(
      async (tx) => {
        const rfqs = await tx
          .select()
          .from(sourcingRfq)
          .orderBy(asc(sourcingRfq.createdAt));
        ...
        const [
          ...
          rfqIds.length
            ? tx
                ...
                .where(inArray(sourcingRfqInvitation.rfqId, rfqIds))
            : [],
          rfqIds.length
            ? tx
                .select()
                .from(sourcingSupplierQuote)
                .where(inArray(sourcingSupplierQuote.rfqId, rfqIds))
            : [],
          rfqIds.length
            ? tx
                .select()
                .from(sourcingAward)
                .where(inArray(sourcingAward.rfqId, rfqIds))
            : [],
          rfqIds.length
            ? tx
                .select()
                .from(rfqInvite)
                .where(inArray(rfqInvite.rfqId, rfqIds))
            : [],
          rfqIds.length
            ? tx
                .select()
                .from(networkQuoteSubmission)
                .where(inArray(networkQuoteSubmission.rfqId, rfqIds))
            : [],
          ...
          rfqIds.length
            ? tx
                ...
                .from(item)
                .where(
                  inArray(item.id, [...new Set(rfqs.map((r) => r.itemId))]),
                )
            : [],
        ]);
        ...
        const identity = (supplierId: string): string =>
          supplierById.get(supplierId)?.vendorId ?? `network:${supplierId}`;
        const requests: DeskRequest[] = rfqs.map((r) => {
          ...
          const mineRecorded = recorded.filter(
            (q) => q.rfqId === r.id && q.status === "submitted",
          );
          ...
          const quotes: DeskQuote[] = [
            ...mineRecorded.map((q) => ({
              ...
              eligibleForAward: awardBlockers(r, q).length === 0,
              meetsNeedDate: q.promisedDate
                ? q.promisedDate <= r.needDate
                : null,
            ...
            ...mineSubs
              ...
              .map((s) => ({
                ...
                supplierName:
                  supplierById.get(s.supplierId)?.name ?? "Supplier",
                ...
                meetsNeedDate: s.promisedDate
                  ? s.promisedDate <= r.needDate
                  : null,
                technicalGate: null,
                gateNote: null,
          ...
          const usable = quotes.filter((q) => q.eligibleForAward);
          const best = usable[0] ?? null;
          const cheapest = quotes[0] ?? null;
          const highest = usable.length ? usable[usable.length - 1]! : null;

          for (const q of quotes) {
            q.savingsVsHighest =
              q.eligibleForAward &&
              highest &&
              Number(highest.landedCost) > Number(q.landedCost)
                ? m2(
                    (Number(highest.landedCost) - Number(q.landedCost)) *
                      Number(r.qty),
                  )
          ...
          const responded = new Set([
            ...
            ...mineSubs.map((s) => identity(s.supplierId)),
          ]).size;
          ...
          return {
            ...
            invited: new Set([
              ...invited.map((i) => i.vendorId),
              ...netInvitedHere.map((i) => identity(i.supplierId)),
            ]).size,
            responded,
            quotes,
            bestUsable: best
              ? { supplierName: best.supplierName, landedCost: best.landedCost }
              : null,
        ...
        const deskSuppliers: DeskSupplier[] = suppliers.map((s) => {
          ...
          const answerCount = new Set(answered.map((x) => x.rfqId)).size;
        ...
        const savings = decided.reduce((sum, r) => {
          const eligible = r.quotes.filter((q) => q.eligibleForAward);
          const highest = eligible[eligible.length - 1];
          if (!highest || !r.bestUsable) return sum;
          const delta =
            (Number(highest.landedCost) - Number(r.bestUsable.landedCost)) *
            Number(r.qty);
          return delta > 0 ? sum + delta : sum;
        }, 0);

        const totalInvites = new Set(
          netInvites.map((i) => `${i.rfqId}:${identity(i.supplierId)}`),
        ).size;
        const totalAnswers = new Set(
          submissions.map((i) => `${i.rfqId}:${identity(i.supplierId)}`),
        ).size;

        return {
          kpis: {
            openRequests: requests.filter(
              (r) => r.stage !== "awarded" && r.stage !== "closed",
            ).length,
            awaitingResponse: requests.filter(
              (r) => r.stage === "out_to_market",
            ).length,
            quotesIn: requests.reduce((n, r) => n + r.quotes.length, 0),
            awarded: requests.filter((r) => r.stage === "awarded").length,
            suppliers: suppliers.length,
            responseRate: totalInvites
              ? Math.round((totalAnswers / totalInvites) * 100)
              : null,
