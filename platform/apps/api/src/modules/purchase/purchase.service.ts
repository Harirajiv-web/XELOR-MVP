...
export class PurchaseService implements SupplySource, PurchaseOrderWriter {
          ...
          keyset
            ? and(
                ...
                or(
                  ...
                  and(
                    eq(vendor.createdAt, new Date(keyset.createdAt)),
                    gt(vendor.id, keyset.id),
                  ),
                ),
              )
            : eq(vendor.isActive, true),
  ...
  async createPo(
    ...
    if (input.lines.length === 0) {
      throw Errors.validation([
        { field: "lines", message: "a purchase order needs at least one line" },
      ]);
    }
    const result = await runIdempotent(
      idempotencyKey,
  ...
  private async doCreatePo(input: CreatePoInput): Promise<PoView> {
    return withTenant((tx) => this.createPoInTx(tx, input));
  }

  /** Domain writes reusable inside an atomic source-document conversion. */
  async createPoInTx(tx: Tx, input: CreatePoInput): Promise<PoView> {
    ...
    if (
      ...
      (!Number.isFinite(input.additionalCharges) || input.additionalCharges < 0)
    )
      throw Errors.validation([
        {
          field: "additionalCharges",
          message: "Charges must be finite and non-negative.",
        },
      ]);
    const { tenantId, actorId } = currentTenant();
    const now = new Date();
    ...
    const id = newId();
    const poNo = await this.numbering.next(
      tx,
      "purchase_order",
      fyCode(now.toISOString()),
    );
    let total = round2(input.additionalCharges ?? 0);
    ...
    await tx.insert(purchaseOrder).values({
      ...
      additionalCharges: round2(input.additionalCharges ?? 0).toFixed(2),
  ...
  private async doEditPo(poId: string, input: EditPoInput): Promise<PoView> {
    ...
    return withTenant(async (tx) => {
      ...
      if (input.expectedDate !== undefined) {
        header.expectedDate = input.expectedDate
      ...
      if (input.additionalCharges !== undefined) {
        if (
          !Number.isFinite(input.additionalCharges) ||
        ...
        header.additionalCharges = round2(input.additionalCharges).toFixed(2);
      }
      ...
      if (input.lines) {
        ...
        for (const l of existingLines) {
          ...
          if (qty > 0)
            received.set(l.itemId, (received.get(l.itemId) ?? 0) + qty);
        }
        ...
        newTotal = round2(
          input.lines.reduce((sum, l) => sum + round2(l.qty * l.rate), 0) +
            Number(header.additionalCharges ?? before.additionalCharges),
        );
        header.totalAmount = newTotal.toFixed(2);
      }

      if (!input.lines && input.additionalCharges !== undefined) {
        newTotal = round2(
          existingLines.reduce((sum, line) => sum + Number(line.amount), 0) +
      ...
      let withdrawn = false;
      if (
        outcome.reapprovalRequired &&
        ["pending_approval", "approved", "partially_received"].includes(
          before.status,
        )
      ) {
        ...
        columns.status =
          before.status === "partially_received"
            ? "partially_received"
            : "draft";
          ...
          input.lines.map((l, i) => {
            ...
            return {
              ...
              salesOrderLineId:
                ...
                existingLines.find((e) => e.itemId === l.itemId)
                  ?.salesOrderLineId ??
                null,
      ...
      await tx.insert(outboxEvent).values({
        ...
        payload: {
          ...
          totalAmount:
            newTotal !== null ? newTotal.toFixed(2) : before.totalAmount,
  ...
  async poEditPolicy(poId: string) {
    ...
    return {
      ...this.edits.policy("purchase.po", row.status),
      status: row.status,
    ...
    return withTenant(async (tx) => {
      ...
      return rows.map((r) => ({
        ...
        dueDate: r.expectedDate
          ? r.expectedDate.toISOString().slice(0, 10)
          : null,
        ref: r.poNo,
          ...
          keyset
            ? and(
                ...
                or(
                  ...
                  and(
                    eq(purchaseOrder.createdAt, new Date(keyset.createdAt)),
                    gt(purchaseOrder.id, keyset.id),
                  ),
                ),
              )
            : eq(purchaseOrder.isActive, true),
  ...
  async createGrn(
    ...
    if (input.lines.length === 0) {
      throw Errors.validation([
        { field: "lines", message: "a goods receipt needs at least one line" },
      ]);
    }
    const result = await runIdempotent(
      idempotencyKey,
    ...
    return withTenant(async (tx) => {
      ...
      if (po.status !== "approved" && po.status !== "partially_received") {
        throw new AppError(
          "PO_NOT_RECEIVABLE",
          409,
          `PO is ${po.status}; only an approved PO can be received.`,
        );
      }
      ...
      input.lines.forEach((l, i) => {
        ...
        if (!pl)
          throw new AppError(
            "GRN_LINE_INVALID",
            422,
            `line ${i + 1}: poLineId is not on this PO`,
          );
        ...
        if (l.qty > remaining + 1e-9) {
          throw new AppError(
            "OVER_RECEIPT",
            422,
            `line ${i + 1}: receiving ${l.qty} exceeds remaining ${remaining}`,
          );
        }
        const batch = l.batch ?? "";
        stockLines.push({
      ...
      const grnNo = await this.numbering.next(
        tx,
        "goods_receipt",
        fyCode(grnDate.toISOString()),
      );
      ...
      for (const pl of poLines) {
        const recvThis = input.lines
          .filter((x) => x.poLineId === pl.id)
        ...
        if (recvThis > 0) {
          await tx
            ...
            .set({
              receivedQty: newReceived.toFixed(3),
              updatedBy: actorId,
              updatedAt: now,
            })
