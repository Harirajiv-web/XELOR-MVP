import { lockCommercialInTx } from "../../common/commercial-rules.js";
...
export class SalesService implements DemandSource, CustomerOrderWriter {
  ...
  async createCustomer(
    ...
    const result = await runIdempotent(
      idempotencyKey,
      fingerprint({ ...input, op: "create-customer" }),
      async () => {
  ...
  private async doCreateCustomer(
    ...
    if (registered && !input.gstin) {
      throw Errors.validation([
        { field: "gstin", message: "a registered customer must have a GSTIN" },
      ]);
    }
    if (!registered && input.gstin) {
      throw Errors.validation([
        {
          field: "gstin",
          message: "an unregistered customer must not carry a GSTIN",
        },
      ]);
    }
    ...
    if (input.gstin) {
      ...
      if (!v.ok)
        throw Errors.validation([
          { field: "gstin", message: v.reason ?? "invalid GSTIN" },
        ]);
    ...
    return withTenant(async (tx) => {
      ...
      const row = (
        await tx.select().from(customer).where(eq(customer.id, id)).limit(1)
      )[0]!;
      return {
        outcome: "created",
        customer: {
  ...
  async createOrder(
    ...
    const result = await runIdempotent(
      idempotencyKey,
      fingerprint({ ...input, op: "create-so" }),
      async () => ({
        status: 201,
  ...
  private async doCreateOrder(
    ...
    return withTenant((tx) => this.createOrderInTx(tx, input));
  }

  /** Domain writes reusable inside an atomic source-document conversion. */
  async createOrderInTx(
    ...
    if (input.lines.length === 0) {
      throw Errors.validation([
        { field: "lines", message: "a sales order needs at least one line" },
      ]);
    }
    const { tenantId, actorId } = currentTenant();
    const now = new Date();
    ...
    if (!supplier.ok) {
      throw Errors.validation([
        { field: "supplierGstin", message: supplier.reason ?? "invalid GSTIN" },
      ]);
    }
      ...
      throw Errors.validation([
        {
          ...
          message: "required — the customer has no GSTIN to derive it from",
        },
      ]);
    }
    ...
    const shipTo = checkShipToGstin({
      docDate: orderDate,
      shipToGstin:
        input.shipToGstin ?? (input.shipToStateCode ? undefined : cust.gstin),
      shipToIsRegistered: cust.isRegistered,
      config,
    });
    if (!shipTo.ok) {
      throw new AppError(
        "SHIP_TO_GSTIN_REQUIRED",
        422,
        shipTo.reason ?? "ship-to GSTIN required",
      );
    }
    ...
    const dupPo = await tx
      ...
      .where(
        and(
          eq(salesOrder.customerId, input.customerId),
    ...
    const soNo = await this.numbering.next(
      tx,
      "sales_order",
      fyCode(orderDate),
    );
  ...
  async editOrder(
    ...
    const result = await runIdempotent(
      ...
      async () => ({
        status: 200,
        body: await this.doEditOrder(orderId, input),
      }),
    );
    return result.body;
  }

  private async doEditOrder(
    orderId: string,
    input: EditOrderInput,
  ): Promise<SalesOrderView> {
    ...
    return withTenant(async (tx) => {
      ...
      if (input.shipToStateCode !== undefined)
        header.shipToStateCode = shipToStateCode;
      if (input.shipToGstin !== undefined)
        header.shipToGstin = input.shipToGstin;
      if (input.shipToAddress !== undefined)
        header.shipToAddress = input.shipToAddress;
      if (input.fgWarehouseId !== undefined)
        header.fgWarehouseId = input.fgWarehouseId;

      if (input.shipToStateCode !== undefined) {
        header.placeOfSupply = shipToStateCode;
        header.isInterState =
          stateCodeOfGstin(before.supplierGstin) !== shipToStateCode;
      }
      ...
      if (
        input.shipToGstin !== undefined ||
        input.shipToStateCode !== undefined
      ) {
        const [cust] = await tx
          .select({
      ...
      if (input.lines) {
        ...
        for (const l of existingLines) {
          ...
          if (qty > 0)
            shipped.set(l.itemId, (shipped.get(l.itemId) ?? 0) + qty);
        }
      ...
      const linesChanged =
        ...
        this.linesDiffer(existingLines, input.lines);
      ...
      const valueMoved =
        tax !== null && m2(tax.grandTotal) !== before.grandTotal;
      ...
      await tx.insert(outboxEvent).values({
        ...
        name: eventName(
          "sales",
          "order",
          outcome.tier === "amend" ? "amended" : "corrected",
        ),
        payload: {
          id: orderId,
          soNo: before.soNo,
  ...
  async orderEditPolicy(orderId: string) {
    ...
    return {
      ...this.edits.policy("sales.order", row.status),
      status: row.status,
  ...
  async confirmOrder(
    ...
    const result = await runIdempotent(
      ...
      async () => ({
        status: 201,
        body: await this.doConfirmOrder(orderId, override),
      }),
    );
    return result.body;
  }
    ...
    return withTenant(async (tx) => {
      ...
      const cust = (
        await tx
          .select()
          .from(customer)
          .where(eq(customer.id, so.customerId))
          .limit(1)
      )[0]!;
      ...
      const ar = await this.accounts.getCustomerOutstandingInTx(
        tx,
        so.customerId,
      );
  ...
  async dispatchOrder(
    ...
    const result = await runIdempotent(
      ...
      async () => ({
        status: 201,
        body: await this.doDispatch(orderId, input),
      }),
    );
    return result.body;
  }
  ...
  private async doDispatch(
    ...
    if (input.lines.length === 0) {
      throw Errors.validation([
        { field: "lines", message: "a dispatch needs at least one line" },
      ]);
    }
    const { tenantId, actorId } = currentTenant();
    const now = new Date();
    ...
    return withTenant(async (tx) => {
      ...
      if (so.status === "credit_hold") {
        throw new AppError(
          "ORDER_ON_CREDIT_HOLD",
          409,
          `Order ${so.soNo} is on credit hold and cannot ship.`,
        );
      }
      ...
      if (!so.fgWarehouseId) {
        throw Errors.validation([
          {
            field: "fgWarehouseId",
            message: "the order has no source warehouse",
          },
        ]);
      }
      ...
      for (const req of input.lines) {
        ...
        if (!line)
          throw new AppError(
            ...
            422,
            `Line ${req.orderLineId} is not on ${so.soNo}.`,
          );
        if (req.qty <= 0)
          throw Errors.validation([{ field: "qty", message: "must be > 0" }]);
        ...
        stockLines.push({
          itemId: line.itemId,
          fromWarehouseId: so.fgWarehouseId,
          qty: req.qty,
        });
      }
      ...
      const dispatchNo = await this.numbering.next(
        tx,
        "delivery_note",
      ...
      const allDone = after.every(
        (l) => numOf(l.deliveredQty) + 1e-9 >= numOf(l.qty),
      );
  ...
  async listOrders(
    limit: number,
    cursor?: string,
  ): Promise<CursorPage<SalesOrderSummary>> {
    return withTenant(async (tx) => {
          ...
          keyset
            ? and(
                ...
                or(
                  ...
                  and(
                    eq(salesOrder.createdAt, new Date(keyset.createdAt)),
                    gt(salesOrder.id, keyset.id),
                  ),
                ),
              )
            : eq(salesOrder.isActive, true),
      ...
      const lineRows =
        ids.length === 0
          ...
          : await tx
              ...
              .where(
                and(
                  inArray(salesOrderLine.orderId, ids),
                  eq(salesOrderLine.isActive, true),
                ),
              );
          ...
          keyset
            ? and(
                ...
                or(
                  ...
                  and(
                    eq(customer.createdAt, new Date(keyset.createdAt)),
                    gt(customer.id, keyset.id),
                  ),
                ),
              )
            : eq(customer.isActive, true),
  ...
  private async viewInTx(tx: Tx, id: string): Promise<SalesOrderView> {
    ...
    const cust = (
      await tx
        .select()
        .from(customer)
        .where(eq(customer.id, r.customerId))
        .limit(1)
    )[0];
    ...
    const specs =
      itemIds.length === 0 ? [] : await this.items.getItems(itemIds);
    ...
    for (const l of lines) {
      ...
      if (!nextPromise || l.requestedDeliveryDate < nextPromise)
        nextPromise = l.requestedDeliveryDate;
    }
   ...
   */
  private async lastSoldTerms(
    tx: Tx,
    itemId: string,
    customerId: string | null,
  ): Promise<{ rate: number; hsn: string; gstRatePct: number } | null> {
    ...
    for (const scoped of customerId ? [true, false] : [false]) {
      const rows = await tx
        .select({
          rate: salesOrderLine.rate,
          hsn: salesOrderLine.hsn,
          gst: salesOrderLine.gstRatePct,
          orderDate: salesOrder.orderDate,
          createdAt: salesOrderLine.createdAt,
        })
        .from(salesOrderLine)
        .innerJoin(salesOrder, eq(salesOrder.id, salesOrderLine.orderId))
        .where(
        ...
        .orderBy(
          sql`${salesOrder.orderDate} desc, ${salesOrderLine.createdAt} desc`,
        )
        .limit(1);
      const hit = rows[0];
      if (hit)
  ...
  private async sellingGstin(tx: Tx): Promise<string> {
    const recent = await tx
      .select({ gstin: salesOrder.supplierGstin })
      .from(salesOrder)
      .orderBy(sql`${salesOrder.orderDate} desc`)
      .limit(1);
    if (recent[0]?.gstin) return String(recent[0].gstin);
    const reg = await tx
      .select({ gstin: gstRegistration.gstin })
      .from(gstRegistration)
      .orderBy(asc(gstRegistration.gstin))
      .limit(1);
    if (!reg[0]) {
      throw new AppError(
        "NO_GST_REGISTRATION",
  ...
  private async finishedGoodsWarehouse(tx: Tx): Promise<string | undefined> {
    const recent = await tx
      .select({ id: salesOrder.fgWarehouseId })
      .from(salesOrder)
  ...
  async createConfirmedOrder(
    ...
    if (input.lines.length === 0) {
      throw Errors.validation([
        { field: "lines", message: "an order needs at least one line" },
      ]);
    }
      ...
      for (const line of input.lines) {
        ...
        if (!spec) {
          throw new AppError(
            "ITEM_NOT_FOUND",
            404,
            `No item ${line.itemId} in the master.`,
          );
        }
        const supplied =
          line.rate !== undefined &&
          line.hsn !== undefined &&
          line.gstRatePct !== undefined;
        const terms = supplied
          ? {
              rate: line.rate as number,
              hsn: line.hsn as string,
              gstRatePct: line.gstRatePct as number,
            }
  ...
  async listOrderableCustomers(): Promise<
  ...
  async listSellableItems(): Promise<
    Array<{
      id: string;
      itemCode: string;
      name: string;
  ...
  > {
    return withTenant(async (tx) => {
      const rows = await tx
        .select({
          id: itemTable.id,
          itemCode: itemTable.itemCode,
          name: itemTable.name,
          uom: itemTable.uom,
        })
        .from(itemTable)
        .where(
          and(eq(itemTable.isSellable, true), eq(itemTable.isActive, true)),
        )
