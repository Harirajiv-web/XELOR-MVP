...
const { salesQuotation, salesQuotationLine, customer, item, outboxEvent } =
  schema;
 ...
 *
 *   Conversion happens ONCE. The source lock, order creation and quotation update share
 *   one transaction, so separate API instances cannot create orphan duplicate orders.
 */
@Injectable()
export class QuotationService {
  ...
  private async doCreate(input: CreateQuotationInput): Promise<QuotationView> {
    if (input.lines.length === 0) {
      throw Errors.validation([
        { field: "lines", message: "a quotation needs at least one line" },
      ]);
    }
    ...
    if (input.validUntil < quoteDate) {
      throw Errors.validation([
        {
          field: "validUntil",
          message: "a quotation cannot expire before the day it was raised",
        },
      ]);
    }
    ...
    return withTenant(async (tx) => {
      ...
      const id = newId();
      const quoteNo = await this.numbering.next(
        tx,
        "sales_quotation",
        fyCode(quoteDate),
      );
      const { lines, subtotal, taxTotal } = this.priceLines(
        input.lines,
        id,
        tenantId,
        actorId,
  ...
  async revise(
    ...
    const result = await runIdempotent(
      ...
      async () => ({
        status: 201,
        body: await this.doRevise(quotationId, input),
      }),
    );
    return result.body;
  }

  private async doRevise(
    quotationId: string,
    input: CreateQuotationInput,
  ): Promise<QuotationView> {
    if (input.lines.length === 0) {
    ...
    return withTenant(async (tx) => {
      ...
      if (["superseded", "accepted"].includes(prior.status))
        throw new AppError(
          "QUOTATION_NOT_REVISABLE",
          409,
          "Only an unaccepted current revision may be repriced.",
        );
      if (input.validUntil < (input.quoteDate ?? today()))
        throw Errors.validation([
          {
            field: "validUntil",
            message: "Expiry cannot precede the quotation date.",
          },
        ]);
  ...
  async send(
    ...
    const result = await runIdempotent(
      ...
      async () => ({
        status: 200,
        body: await this.transition(quotationId, ["draft"], "sent", "sent"),
      }),
    );
    return result.body;
  }
  ...
  async convert(
    ...
    const result = await runIdempotent(
      ...
      async () => ({
        status: 201,
        body: await this.doConvert(quotationId, input),
      }),
    );
    return result.body;
  }
  ...
  private async doConvert(
    ...
    return withTenant(async (tx) => {
      await lockCommercialInTx(tx);
      await tx
        .select({ id: salesQuotation.id })
        .from(salesQuotation)
        .where(eq(salesQuotation.id, quotationId))
        .for("update");
      const quote = await this.viewInTx(tx, quotationId);
      ...
      if (quote.expired)
        throw new AppError(
          "QUOTATION_EXPIRED",
          409,
          "Revise the expired quotation before converting it.",
        );
      ...
      const order = await this.sales.createOrderInTx(tx, {
  ...
  async list(
    limit: number,
    cursor?: string,
  ): Promise<CursorPage<QuotationSummary>> {
    return withTenant(async (tx) => {
  ...
  private priceLines(
    ...
    lines: (typeof salesQuotationLine.$inferInsert)[];
    subtotal: number;
    taxTotal: number;
  } {
    let subtotal = 0;
    ...
    const lines = input.map((l, i) => {
      if (l.qty <= 0) {
        throw Errors.validation([
          { field: `lines.${i}.qty`, message: "quantity must be positive" },
        ]);
      }
      if (l.rate < 0) {
        throw Errors.validation([
          { field: `lines.${i}.rate`, message: "rate cannot be negative" },
        ]);
      }
  ...
  private async load(
    tx: Tx,
    quotationId: string,
  ): Promise<typeof salesQuotation.$inferSelect> {
    await lockCommercialInTx(tx);
    const [row] = await tx
      .select()
      .from(salesQuotation)
