...
export const purchaseOrder = pgTable(
  "purchase_order",
  {
    ...
    totalAmount: numeric("total_amount", { precision: 18, scale: 2 })
      .notNull()
      .default("0"),
    /** Order-level tooling, freight and non-creditable charges included in approval value. */
    additionalCharges: numeric("additional_charges", {
      precision: 18,
      scale: 2,
    })
      .notNull()
...
export const grn = pgTable(
  ...
  (t) => [
    unique("uq_grn_tenant_no").on(t.tenantId, t.grnNo),
    index("ix_grn_tenant_po").on(t.tenantId, t.poId),
  ],
);
