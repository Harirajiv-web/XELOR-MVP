...
The six live capability boundaries are:
...
All six call existing domain services and re-check the requesting user's permission.

## Mission Control

`/agentos/command` provides:

- a catalogue-backed seven-agent topology;
...
identity, enters the ONYX gateway, asserts that all seven agents are connected, opens
